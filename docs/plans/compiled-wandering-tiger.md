...
## Рефакторинг (расширение скоупа) — консервативный, тестируемый

Baseline тестов на `:next`: **183 теста, все OK** (7 skip integration) после создания `storage/temp`.

**Взято (низкий риск, поведение не меняется):**
1. Извлечь чистые функции Main.py → `webui/utils/text_utils.py` (`parse_chatterbox_voices`, `detect_audio_mime`) + новые тесты `test_text_utils.py`.
2. Магические числа → константы: `material.py` (per_page, API_TIMEOUT, User-Agent), `voice.py` (коэффициенты длительности).
3. Мелкие фиксы `video.py`: удалить no-op `clip = clip`, улучшить лог в error-path.
4. Фикс хрупкого теста `test_gemini_tts_uses_legacy_submaker_fields` (создавать `storage/temp`).

**Отклонено (высокий риск / не покрыто юнит-тестами — форк в проде):**
- UI-модуляризация Main.py (фазы 4-5 карты): не тестируется юнитами, ломает прод.
- Разбивка `combine_videos()` (209 строк, НЕ покрыта тестами) — критично.
- Рефакторинг `_generate_response()` (460 строк) — слишком рискованно.

Каждый блок делает отдельный агент (непересекающиеся файлы), затем полный прогон тестов в контейнере + code-review.
